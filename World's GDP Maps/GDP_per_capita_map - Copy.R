# Mapa de Gross Domestic Product (GDP) per capita
...
# separar as variáveis “country” e “gdp_per_capita” (criar um subset)

econ <- subset(economia, select=c(country, gdp_per_capita))
...
m1 <- m0 + geom_polygon(aes(x=long, y=lat, group=group, fill= gdp_per_capita)) + coord_equal()
...
ggsave("GDP_per_capita_map.png", plot = m3, width = 7, height = 3.5, units = "in", dpi = 1500)